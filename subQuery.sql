...
-- when using subquery inside SELECT clause, we can only use those subquery which returns a single value
SELECT *, (SELECT sum(salary) from employees2) from employees2;

-- using inside FROM clause, subquery should be compatible with outer query
SELECT department_name, sum(salary) FROM employees2 GROUP BY department_name;
-- how to do the above thing using subquery
SELECT * FROM 
(SELECT department_name, sum(salary) FROM employees2 GROUP BY department_name) 
as sum_dept_salary;

-- using inside WHERE clause, it will depend on what type of comparison operator I am using
SELECT * FROM employees2
WHERE 
salary > (SELECT  max(salary) FROM employees2 WHERE department_name = 'HR');