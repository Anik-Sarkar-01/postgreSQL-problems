...
-- the same thing can be written in another way - USING keyword
SELECT * FROM employees
...
SELECT extract(year from hire_date) hire_year, count(*) FROM employees
GROUP BY hire_year ORDER BY hire_year ASC
...
--6: Find customers who have placed more than 2 orders and calculate 
-- the total amount spent by each of these customers.
SELECT customer_id, count(*) as order_count, sum(total_amount) FROM orders
GROUP BY customer_id
HAVING count(*) > 2

--  7: Find the total amount of orders placed each month in the year 2022.
SELECT extract(month from order_date), sum(total_amount) FROM orders
